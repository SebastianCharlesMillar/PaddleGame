class Ball:
    ...
    def draw(self, pad_speed):
        ...
        if self.pos_y > height-50 - self.sz/2: #if ball y position becomes greater than the height minus 50 minus half the ball size
            if self.paddle_hit_detection():     #if ball hits the paddle
                if pad_speed * self.dx > 0 and abs(self.dx) <3.5:
                    self.dx *= 1.4
                elif pad_speed * self.dx < 0 and abs(self.dx) > 0.5:
                    self.dx /= 1.4
                else:
                    self.dx = self.dx
    ...
    def paddle_hit_detection(self):
        if self.pos_x > self.padpos - 25 and self.pos_x < self.padpos + 23 and self.pos_y < height-50 - self.sz/2 + 2 *self.dy: #if ball position x is greater than the paddle poition minus 20, and is less than the paddle position plus 20 and the ball y position is greater than the height les 49(the height where the paddle is)
